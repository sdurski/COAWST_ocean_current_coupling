...
!
!svn $Id: ocean_inlet_test.in 838 2008-11-17 04:22:18Z jcwarner $
!========================================================= Hernan G. Arango ===
...
! Application title.
       ...
       TITLE = Calif. Current System - COAWST3  Coupled
...
! first so all information arrays can be initialized properly.
     ...
     VARNAME = /nobackup/sdurski/CCS_COAWST/ROMS/External/varinfo_mod.dat
...
! these parameters correctly.
          ...
          Lm ==  555           ! Number of I-direction INTERIOR RHO-points
          Mm ==  858           ! Number of J-direction INTERIOR RHO-points
...
!                   1       2       3       4
   ...
   LBC(isFsur) ==   Clo     Clo     Clo     Cha        ! free-surface
...
! Wec boundary conditions
   ...
   LBC(isU2Sd) ==   Gra     Gra     Gra     Clo        ! 2D U-stokes
   LBC(isV2Sd) ==   Gra     Gra     Gra     Clo        ! 2D V-stokes
   LBC(isU3Sd) ==   Gra     Gra     Gra     Clo        ! 3D U-stokes
   LBC(isV3Sd) ==   Gra     Gra     Gra     Clo        ! 3D V-stokes
...
!      NTIMES == 14400    ! 10 days with DT=60
!      NTIMES == 480      ! 1 days
!      NTIMES == 2160     ! 3 days

!    For DT = 180 we need NTIMES= 480 for each day
!     NTIMES == 960        ! 2 days
!      NTIMES == 123360     ! 257 days
!      NTIMES == 50400      ! 105 days - from Sept 18-Dec 31
!      NTIMES == 4800       ! 10 days
!      NTIMES == 480      ! 1 days
!      NTIMES == 181440    ! 378 days - 208 days from new start (Mar 7-Sep 30)
!      NTIMES == 12480      ! 25  days For March 2009 starting on the 7th

!      NTIMES == 189120     !224 days March 7, 2009 (day 170) = 394 days
!      NTIMES == 188160     ! run through the end of October 14th.
       NTIMES == 2880 
          DT == 180.0d0
     NDTFAST == 50
...
! CSD changed from 1e-3.  The above comment really needs to be rewritten
! CSD given that people can change the value below.
   AKT_LIMIT == 1.0d-1 1.0d-1                   ! m2/s

   AKV_LIMIT == 1.0d-1                          ! m2/s
...
! NetCDF units attribute.
  ...
  TIDE_START =  0.0d0                      ! days
    TIME_REF =  20080918.0                      ! yyyymmdd.dd
...
Hout(idUcaw) == T       ! u_caw              surf.lay.est of u-vel for atm. cpl.
Hout(idVcaw) == T       ! v_caw              surf.lay.est of v-vel for atm. cpl.
...
Aout(idUcaw) == T       ! u_caw              surf.lay.est of u-vel for atm. cpl.
Aout(idVcaw) == T       ! v_caw              surf.lay.est of v-vel for atm. cpl.
...
! Input NetCDF file names, [1:Ngrids].
     ...
     GRDNAME == /nobackup/sdurski/CCS_COAWST/Projects/CCS/Setup/grd_or2km_Final_2_SP08.nc
     ININAME == /nobackup/sdurski/CCS_COAWST/Projects/CCS/Setup/no_motion.nc
...
! the script "matlab/grid/contact.m" from the Matlab repository.
     ...
     NGCNAME =  /nobackup/pbarbour/COAWST2/CCS7_input/CCS7_ngc.nc
...
! except the last one.
     ...
     CLMNAME ==/nobackup/pbarbour/COAWST3/CCS8_input/ocean_clm.nc

     NBCFILES == 1                          ! number of boundary files
     BRYNAME == /nobackup/sdurski/CCS_COAWST/Projects/CCS/Setup/NEp_2008_2010_vbrev_bndry.nc
...
! Input Sources/Sinks forcing (like river runoff) file name.
     ...
     SSFNAME == /nobackup/pbarbour/COAWST3/CCS8_inpuocean_rivers.nc
...
! entry, except the last one.
     ...
     FRCNAME == /nobackup/pbarbour/COAWST3/CCS8_input/NEP_NAM_2008_heat_frc.nc | 
                /nobackup/pbarbour/COAWST3/CCS8_input/NEP_NAM_2009_heat_frc.nc \ 
                /nobackup/pbarbour/COAWST3/CCS8_input/NEP_NAM_rt_2008_wind_frc.nc |
                /nobackup/pbarbour/COAWST3/CCS8_input/NEP_NAM_rt_2009_wind_frc.nc

! forcing file 1, grid 1

! Output NetCDF file names, [1:Ngrids].

     GSTNAME == CCS_output/ocean_gst.nc
     RSTNAME == CCS_output/ocean_CCS8_mynn_base_rst.nc 
     HISNAME == CCS_output/ocean_CCS8_mynn_base_his.nc
     TLMNAME == CCS_output/ocean_tlm.nc
     TLFNAME == CCS_output/ocean_tlf.nc
     ADJNAME == CCS_output/ocean_adj.nc
     AVGNAME == CCS_output/ocean_CCS8_mynn_base_avg.nc
     DIANAME == CCS_output/ocean_dia.nc
     STANAME == CCS_output/ocean_sta.nc
     FLTNAME == CCS_output/ocean_flt.nc

! Input ASCII parameter filenames.

     APARNAM =  /nobackup/pbarbour/COAWST3/ROMS/External/s4dvar.in
     SPOSNAM =  /nobackup/pbarbour/COAWST3/ROMS/External/stations.in
     FPOSNAM =  /nobackup/pbarbour/COAWST3/ROMS/External/floats.in
     BPARNAM =  /nobackup/pbarbour/COAWST3/ROMS/External/bioFasham.in
     SPARNAM =  /nobackup/pbarbour/COAWST3/ROMS/External/sediment.in 
     USRNAME =  /nobackup/pbarbour/COAWST3/ROMS/External/MyFile.dat
...
!
!------------------------------------------------------------------------------
! Constants used in the wave effects on currents formulations.
!------------------------------------------------------------------------------
!
!  WEC_ALPHA     If 0 then all wave dissip goes to break and none to roller.
!                If 1 then all wave dissip goes to roller and none to breaking.
!
!------------------------------------------------------------------------------
...
!
!  Hout(idU2rs)  Write out 2D WEC U-stress.
!  Hout(idV2rs)  Write out 2D WEC V-stress.
!  Hout(idU3rs)  Write out 3D WEC U-stress.
!  Hout(idV3rs)  Write out 3D WEC V-stress.
!
...
!  Hout(idW3St)  Write out 3D W-Stokes velocity.
!  Hout(idW3Sd)  Write out 3D Omega-Stokes velocity.
!
...
!  Hout(idUwav)  Write out wave-depth avgeraged U-velocity.
!  Hout(idVwav)  Write out wave-depth avgeraged V-velocity.
!  Hout(idWdif)  Write out wave dissipation due to bottom friction.
!  Hout(idWdib)  Write out wave dissipation due to breaking.
!  Hout(idWdiw)  Write out wave dissipation due to white capping.
!  Hout(idWdis)  Write out wave roller dissipation.
!  Hout(idWrol)  Write out wave roller action density.
!  Hout(idWztw)  Write out WEC quasi-static sea level adjustment.
!  Hout(idWqsp)  Write out WEC quasi-static pressure.
!  Hout(idWbeh)  Write out WEC Bernoulli head.
!
